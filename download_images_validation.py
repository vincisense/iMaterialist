...
class Data(object):
    ...
    def create_validation_directory(self, file_path):
        # create test directory to store all test outputs

        try:
            print('directory to be created is ' + os.path.join(file_path, "data", "validation_images"))
            os.makedirs(os.path.join(file_path, "data", "validation_images"))
        except Exception as e:
            print(e)
            shutil.rmtree(os.path.join(file_path, "data", "validation_images"))
            print('directory creation problem, abort!')
            sys.exit()
    ...
    def download_validation_data(self, validation_json, file_path):

        for index in range(0, len(validation_json['images'])):

            try:
                url = validation_json['images'][index]['url']
                image_id = validation_json['images'][index]['image_id']
                # not all images end with jpg (extract image extension from url)
                label_id = validation_json['annotations'][index]['label_id']

                # add the label to know the class name for each images
                image_name = str(label_id) + '_' + str(url[0]).split('/')[-1]
                ...
                with open(os.path.join(file_path, "data", "validation_images", str(image_name)), 'wb') as handler:
...
def main():
    ...
    data_obj.create_validation_directory(file_path)
    data_obj.download_validation_data(validation_json, file_path)
